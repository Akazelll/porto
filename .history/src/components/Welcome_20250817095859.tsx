...
export function Welcome() {
  ...
  useEffect(() => {
    // Pastikan komponen sudah ter-mount di client sebelum memulai animasi
    setIsMounted(true);

    // Atur timer untuk memulai animasi keluar setelah 2 detik
    const exitTimer = setTimeout(() => {
      setIsExiting(true);
    }, 2000); // Anda bisa mengubah durasi ini (dalam milidetik)

    // Bersihkan timer saat komponen di-unmount
    return () => clearTimeout(exitTimer);
  }, []);

  // Jangan render apa pun di server untuk menghindari kedipan
  if (!isMounted) {
    return null;
  }
